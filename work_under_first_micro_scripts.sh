...
# в которые можно навсегда записать переменные окружения и при перезапуске консоли не удалаяются
...
today=`date +'%d-%m'` #специальные кавычки бэктики применяется в скрипте 
today=$(date +'%d-%m')
today=$(date +'%d-%m') ; echo $today #однострочная команда вывода в кобках применяется в баше
...
# ping -c4 ya.ru проверка соединения с доменным именем - трасировка? с4 - количество запросов
...
# dig ya.ru @8.8.8.8 #######################       уточнить                    ######################
# di +trace ya.ru #################                уточнить                        ##################
...
alias - назначение нового названия для команды с ключем например
alias lsa="ls -la"
alias pss="ps -auxf"
что бы алиасы не исчезли их нужно добавить в .bashrc
nano .bashrc
...
ping -c1000 ya.ru > /tmp/ping.txt $
направляем запрос \ количество запросов \ куда направляем \ куда записываем \  $ - создание работы запроса в фоновом режиме

bg 2 ------ background и номер фоновой задачи которая приостановлена
jobs ------ возобновление приостановленны задач
fg 2 ------ forground перевод задачи из фоновой в преоритетный
